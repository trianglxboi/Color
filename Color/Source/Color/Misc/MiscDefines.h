...
#define CL_BIND_METHOD_ON_OBJECT_ON_CLASS(pObject, classToUse, method) [pObject](auto&&... args) -> decltype(auto) { return pObject->classToUse::method(std::forward<decltype(args)>(args)...); }
#define CL_BIND_METHOD_ON_OBJECT(pObject, method) [pObject](auto&&... args) -> decltype(auto) { return pObject->method(std::forward<decltype(args)>(args)...); }
